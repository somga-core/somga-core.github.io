...
Каждое предложение строится по данной структуре: `<время><подлежащее><разделяющее слово><сказуемое>`
 ...
 - `<разделяющее слово>` может быть:
  - `*` если сказуемым является `<действие>` (в этом случае `<сказуемое>` совершает действие `<подлежащее>`)
  - `=` если сказуемым является `<объект>` (в этом случае `<сказуемое>` является `<подлежащим>` (значение слова "есть"))
...
### Грамматика
Каждое предложение строиться по данной структуре: `<время><подлежащее>*<сказуемое>` (`<сказуемое>` совершает действие `<подлежащее>`)
 - `<время>` может быть:
  - `+-` если действие происходит сейчас
  - `++` если действие будет происходить в будущем
  - `--` если действие произошло в прошлом
...
Пример: 90.000.000 (дышинышушуш), 9.000.340 (дышушушитышикышиныш), 8.200.087 (вышушидышинышинышушивышисыш)

Запятая (плавающая точка) произносится "аш". Перед ней соединительная "и" также не ставится

Пример: 3.1415926 (тышашилышикышилышипышидышивышишиши)
