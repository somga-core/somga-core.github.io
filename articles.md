...
# Язык 1119
1119 (Чк чк чк пщщ) - минималистичный язык, вдохновлённый языками 911 и 9111. Он отличается абсолютной однозначностью высказываний и отсутствием специальных символов в письменности (Записывается только цифрами)
...
Регистровый слой - декларативно построенный список регистров. Регистры являются какими-либо существующими или несуществующими объектами в какой-либо момент времени (Они не могут быть чем-то абстрактным, как, например, "движение" или "любовь", только материальным, как, например, "человек" или "элементарная частица". Исключением являются множества, однако они по-сути являются несколькими объектами, рассмотренными вместе). Они называются регистрами, потому что хранят в себе список взаимодействий с другими регистрами. Одно взаимодействие происходит максимум между двумя регистрами
...
Все регистры автоматически "выбрасываются" если явно понятно, что мысль закончена. Например при длительном молчании говорящего или при окончании места для надписи (Например, если на кнопке не написано никаких слов кроме тех, что не "выбрасывают" регистры, то они выбрасываются после всего, что написано на кнопке). Это нужно в основном для более компактной записи

В регистровом слое, помимо регистров так же есть нуллы и числа. Числа нужны для обозначения количественного свойства какого-либо взаимодействия (Например, для указания того, из какого количества элементов состоит множество)
...
Так же `<объект>` может быть цифрой от 1 до 9, означающей количество четвёрок разрядов числа, которое будет указано после `<взаимодействия>`. Например `2130 0231` означает, что регистр 2 имеет красный канал цвета, величиной в число с одной четвёркой разрядов, разделённое на 255 (`2130`): 231 (`0231`)

Так же `<объект>` может быть числом от 1 до 9

Выбор трактования `<объекта>` (Как цифры, числа или как регистра) зависит от значения `<слова>` в словаре. Например, у слова `30` первое место - объект, а второе - число. Поэтому первый `<объект>` всегда будет означать регистр или нулл, а второй `<объект>` - количество четвёрок разрядов числа 

Ещё пример текста: `1040 1133 0000` (Регистр 1 - животное. Регистр 1 имеет непрозрачность величиной в 0 из 99; Животное невидимое)

## Словарь
В таблице ниже приведены значения `<слов>`. В их описании вы можете заметить конструции "о1", "р1", "н1", "ч1", "ц1" и "о2", "р2", "н2", "ч2", "ц2". Это обозначения трактовки `<объектов>`. Те, что заканчиваются на "1" - `<объект>` на первом месте. Те, что на "2" - `<объект>` на втором месте (Например, во `<взаимодействии>` `3410`, `3` - это `<объект>` на первом месте, а `4` - на втором)

Начинающиеся на "о" означают регистр от 1 до 9 или нулл (`0` - `9`). На "р" - регистр от 1 до 9 (`1` - `9`). На "н" - нулл (`0`). На "ч" - число после `<высказывания>` (`1` - `9` с числом после `<высказывания>`). На "ц" - число (`1` - `9`)

Через ИЛИ перечисляются различные значения `<слова>` при разных `<объектах>` - нуллах или не нуллах (То самое, что упоминалось в семантике)

|Слово|Значение|
|:-|:-|
|10|о1 является мною. о2 является тобою|
...
## Фонетика и жесты
Каждая цифра соответсвует определённому звуку и жесту. Цифры читаются по порядку

|Цфр|Звук|Жест|
|:-|:-|:-|
|0|Звукоподражание вращающегося винта пропеллера|Волнообразное движение пальцами с повёрнутой вниз ладонью кистью|
|1|"Чк"|Движение вертикально повёрнутым кулаком вниз и вверх|
|2|Свист|Движение кистью от себя, затем к себе. В ближайшей к телу точке кисть повёрнута ладонью вверх, в самой дальней - ладонью вниз|
...
|6|"У" произнесённый в глотке|Движение кулаком влево и вправо (Вправо и влево, если делать жест левой рукой)|
|7|Шмыганье носом|Движение кистью от себя, затем к себе. Пальцы показывают "козу". В ближайшей к телу точке кисть повёрнута ладонью вверх, в самой дальней - ладонью вниз|
|8|Цоконье языком|Щелбан|
|9|"Пщщ"|Отбрасывание ладони вверх и от себя в бок|

## Примеры
Классика:
- `1210 1269` (Регистр 1 является мною. Регистр 2 является тобою. Регистр 1 имеет половые отношения с регистром 2; Я имею половые отношения с тобой)